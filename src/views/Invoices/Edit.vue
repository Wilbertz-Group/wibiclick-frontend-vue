// wibiclick-frontend-vue/src/views/Invoices/Edit.vue
<script setup>
...
async function fetchInvoices() {
  try {
    ...
    for (const invoice of response.data.invoices) {
      invoices_[invoice.id] = invoice.name
    }

    invoicesData.value = invoices_
    loading.value = false
...
async function sendAttachment(data) {
  ...
  let payload = {
    id: invoiceData.value.id,
    reason: invoice.value.invoice_status,
    name: invoice.value.name,
    ...
    sales: invoice.value.invoice_subtotal,
    subtotal: invoice.value.invoice_subtotal,
    deposit: invoice.value.paid,  
...
<template>
  ...
  <div class="mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
      <main class="detail">
        <modal v-if="isOpen" :website="userStore.currentWebsite" :body="body" :isOpen="isOpen" :blob="blob" :client="client" :sender="sender" :company="company" name="Invoice" :invoiceNumber="invoice.invoice_nr" :totalAmount="invoice.subtotal" :dueDate="invoice.invoice_due_date" :phone="invoice.customer.phone" @close-modal="closeModal"></modal>
        <FormKit
          type="form"
          id="edit"
          ...
          <div class="pl-5 pr-10 pt-5">
            ...
            <!-- <div class="grid grid-flow-col grid-rows-1 grid-cols-12 gap-4 mb-2 border-2 border-transparent border-b-[#11101e]">
              <div class="text-md col-span-8 pb-2">
                <FormKit :type="autocomplete" :options="searchLineItems" name="item_name" placeholder="Line Item Name" v-model="lineItem.name" :value="lineItem.name" input-class="p-1 m-0 bg-slate-100" :classes="{ outer: 'mb-0 ml-0 w-full', inner: { $reset: true, 'relative p-0 m-0': true } }" />
