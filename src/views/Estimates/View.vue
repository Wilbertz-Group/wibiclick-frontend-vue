  ...
  async function saveestimate(data) {
    ...
    function createestimate(estimate, path) {
      ...
      async function sendBlob(blob) {
        if (!blob) return;
        try {
          const to = '27810072528@s.whatsapp.net'; //estimate.value.customer.phone + '@s.whatsapp.net';
...
<template>
  ...
  <div class="mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
      <main class="detail">
        <modal v-if="isOpen" :website="userStore.currentWebsite" :body="body" :isOpen="isOpen" :blob="blob" :client="client" :sender="sender" :phone="estimate.customer.phone" :company="company" name="Estimate" @close-modal="closeModal"></modal>
