// wibiclick-frontend-vue/src/views/Expenses/EditExpense.vue
<template>
        ...
        <div class="">
          <div class="mt-3 md:col-span-2">
            <div v-if="userStore.currentWebsite" class="shadow p-10 sm:rounded-md sm:overflow-hidden">
              <FormKit type="form" id="expense" submit-label="Update" @submit="updateExpense" :actions="false">
                <h2 class="mb-2 text-2xl text-cyan-900 font-bold">Edit expense details below</h2>
                <hr />
...
async function fetchExpense(expenseId) {
  try {
    loading.value = true;
    const response = await axios.get(`expense/${expenseId}?id=${userStore.currentWebsite}`);
    Object.assign(expense.value, response.data.expense);
    expense.value.date = moment(expense.value.date).format('YYYY-MM-DD');
    loading.value = false;
  } catch (error) {
...
async function updateExpense(data) {
  try {
    loading.value = true;
    const response = await axios.put(`expense/${expense.value.id}?id=${userStore.currentWebsite}`, data);
