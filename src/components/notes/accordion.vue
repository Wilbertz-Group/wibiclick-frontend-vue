...
<template>
	<div v-for="(note) in notes" :key="note.uid" :id="dateTimestamp(created)">	
		...
		<div class="shadow rounded-lg sm:overflow-hidden bg-white mb-4">
			...
			<div :id="'accordion-note-body-'+note.uid" class="hidden" aria-labelledby="accordion-arrow-icon-heading-2">
				<div class="px-3 py-1 rounded-b-md font-light border border-b-0 border-gray-200 dark:border-gray-700">
					<p class="p-2 my-2 text-xs italic font-normal text-black border border-gray-200 rounded-lg dark:border-gray-500 dark:text-gray-300" v-html="note.body"></p>
				</div>
			</div>
		</div> 
	</div>
