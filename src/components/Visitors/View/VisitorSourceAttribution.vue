  ...
  <section class="card-modern overflow-hidden shadow-xl">
    ...
    <div class="bg-gradient-to-b from-gray-900 to-gray-800 p-0">
      ...
      <div v-else-if="!attribution" class="flex flex-col items-center justify-center py-12 px-4 text-center">
        ...
        <p class="text-gray-400 text-sm max-w-xs">No source attribution information is available for this visitor.</p>
      </div>
      ...
      <div v-else class="px-6 py-8">
        <div class="space-y-4">
          <div class="flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            <span class="text-sm text-emerald-300 font-semibold">{{ attribution.firstSource?.source || '-' }}</span>
            <span class="ml-4 text-xs text-gray-400">Category:</span>
            <span class="text-xs bg-cyan-700/20 text-cyan-200 px-2 py-0.5 rounded font-medium uppercase">
              {{ attribution.firstSource?.category || '-' }}
            </span>
          </div>
          <div class="flex flex-col sm:flex-row sm:items-center gap-2">
            <span class="text-xs text-gray-400">Last Source:</span>
            <span class="text-sm text-emerald-300 font-semibold">{{ attribution.lastSource?.source || '-' }}</span>
            <span class="ml-4 text-xs text-gray-400">Category:</span>
            <span class="text-xs bg-cyan-700/20 text-cyan-200 px-2 py-0.5 rounded font-medium uppercase">
              {{ attribution.lastSource?.category || '-' }}
            </span>
          </div>
          <div class="flex flex-col gap-1">
            <span class="text-xs text-gray-400">Referrer URLs:</span>
            <ul class="list-disc list-inside text-sm text-cyan-200">
              <li v-for="(ref, idx) in attribution.referrers" :key="idx">
                <a :href="ref" class="hover:underline text-cyan-300" target="_blank" rel="noopener">{{ ref }}</a>
              </li>
              <li v-if="!attribution.referrers || attribution.referrers.length === 0" class="text-gray-400">-</li>
            </ul>
          </div>
          <div class="flex flex-col gap-1">
            <span class="text-xs text-gray-400">Campaign Data (UTM):</span>
            <div v-if="attribution.utm && Object.keys(attribution.utm).length > 0" class="grid grid-cols-2 gap-2">
              <div v-for="(val, key) in attribution.utm" :key="key" class="flex items-center gap-2">
                <span class="text-xs text-cyan-300 font-medium uppercase">{{ key }}:</span>
...
const fetchAttribution = async () => {
  ...
  try {
    const { data } = await axios.get('/api/track/source-attribution', {
      params: { visitorId: props.visitorId }
    });
...
watch(() => props.visitorId, fetchAttribution);
</script>
