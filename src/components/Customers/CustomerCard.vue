...
const props = defineProps({
  customer: {
    type: Object,
    required: true,
    default: () => ({ name: '', email: '', phone: '', portal: '', foreignID: '' })
  },
  notesContent: {
    type: String,
    default: ''
...
const emit = defineEmits([
  'navigate-back',
  'open-note-modal',
  'save-note', // Note: Actual save logic likely remains in parent via modal interaction
  'open-whatsapp-modal',
  'send-whatsapp', // Note: Actual send logic likely remains in parent via modal interaction
  'view-hubspot',
  'update:notesContent', // For v-model binding in modal
  'update:whatsappContent' // For v-model binding in modal
]);
...
// Placeholder function, actual save triggered by parent modal
function saveNote() {
  // emit('save-note'); // Might emit if needed later
}
...
// Placeholder function, actual send triggered by parent modal
function sendWhatsapp() {
  // emit('send-whatsapp'); // Might emit if needed later
}

// Function to update parent's notes model (needed for v-model binding in modal)
function updateNotesModel(value) {
  emit('update:notesContent', value);
}

// Function to update parent's whatsapp model (needed for v-model binding in modal)
function updateWhatsappModel(value) {
  emit('update:whatsappContent', value);
}
...
<template>
  ...
  <section class="shadow sm:rounded-md sm:overflow-hidden relative">
    <!-- Go to all contacts -->
    <div @click="goBack" class="flex justify-start absolute mt-4 ml-3 cursor-pointer">
    ...
    <div class="flex flex-col items-center pb-10">
        ...
        <div class="flex flex-wrap justify-center mt-4 space-x-2 md:space-x-3 md:mt-6"> <!-- Allow wrapping, center, adjust spacing -->
          ...
          <div class="mb-2"> <!-- Added bottom margin -->
            ...
            <!-- Create a note Modal (Structure only - controlled by parent) -->
            <div id="noteModal" data-modal-placement="bottom-right" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
          ...
          <div class="mb-2"> <!-- Added bottom margin -->
            ...
            <!-- Create a whatsapp Modal (Structure only - controlled by parent) -->
            <div id="whatsappModal" data-modal-placement="bottom-right" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                <div class="relative w-full h-full max-w-2xl md:h-auto">
                    <!-- Modal content -->
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <!-- Modal header -->
