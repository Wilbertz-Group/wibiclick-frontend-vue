...
const props = defineProps({
  rawData: { type: Array, required: true }, // Array of customer objects with createdAt
...
const groupedData = computed(() => {
  ...
  const grouped = {};
  props.rawData.forEach(c => {
    const key = moment(c.createdAt).format(groupFormat);
...
const options = computed(() => {
  ...
  return {
    ...
    dataLabels: {
      enabled: true,
      style: {
        colors: [props.isDarkMode ? '#18181b' : '#fff'] // Text color: dark on light, light on dark
      },
      background: {
        enabled: true,
        borderRadius: 4,
        borderWidth: 0,
        opacity: 0.85,
        color: props.isDarkMode ? '#fff' : '#18181b', // Contrasting background for readability
        foreColor: props.isDarkMode ? '#18181b' : '#fff', // Text color inside background
        padding: 2
      },
