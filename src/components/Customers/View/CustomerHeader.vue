...
<template>
  ...
  <header class="mb-8 md:mb-12">
    <!-- Top section with back button and customer name - full width on mobile -->
    <div class="flex flex-wrap items-start mb-4 sm:mb-6">
      <div class="flex items-center space-x-3 w-full">
        <button @click="emit('navigate-back')" class="btn-icon-modern flex-shrink-0" title="Go Back">
          <font-awesome-icon icon="arrow-left" />
        </button>
        <div class="min-w-0 flex-1">
          <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 dark:text-white flex items-center flex-wrap">
            <span class="truncate mr-2">{{ customer.name || 'Loading Customer...' }}</span>              
            <a v-if="customer.portal && customer.foreignID" 
              :href="'https://app.hubspot.com/contacts/'+customer.portal+'/contact/'+customer.foreignID" 
              target="_blank" 
              class="flex-shrink-0 mt-1 sm:mt-0 text-gray-400 hover:text-indigo-500 dark:hover:text-indigo-400" 
              title="View on HubSpot">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 sm:w-10 sm:h-10 cursor-pointer rounded-full shadow bg-[#ff7a59] hover:bg-[#ff7a59] p-2 text-white"
                viewBox="0,0,256,256">
    ...
    <div class="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:space-x-2">
      <button @click="emit('reload-data')" 
        class="btn-icon-modern col-span-2 sm:col-span-1 flex justify-center sm:justify-start" 
        title="Refresh Data" 
        aria-label="Refresh customer data">
        <font-awesome-icon icon="sync" :class="{ 'fa-spin': isLoading }" />
      </button>
      
      <button @click="emit('open-note-modal')" 
        type="button" 
        class="btn-secondary-modern flex justify-center items-center" 
        title="Add Note">
        <font-awesome-icon icon="sticky-note" class="mr-1.5 h-4 w-4" /> 
        <span class="hidden xs:inline">Note</span>
      </button>
      
      <button @click="emit('open-whatsapp-modal')" 
        type="button" 
        class="btn-secondary-modern flex justify-center items-center" 
        title="Send WhatsApp">
        <font-awesome-icon :icon="['fab', 'whatsapp']" class="mr-1.5 h-4 w-4" /> 
        <span class="hidden xs:inline">WhatsApp</span>
      </button>
      
      <!-- AI Job Booking Button -->
      <button @click="emit('ai-book-job')" 
        type="button" 
        class="btn-secondary-modern bg-purple-600 hover:bg-purple-700 border-purple-700 text-white flex justify-center items-center" 
        title="AI Job Booking">
        <font-awesome-icon icon="magic" class="mr-1.5 h-4 w-4" /> 
        <span class="hidden xs:inline">AI Book</span>
        <span class="xs:hidden">AI</span>
      </button>
      
      <button @click="emit('open-add-job-modal')" 
        class="btn-primary-modern flex justify-center items-center" 
        title="Add New Job"> 
        <font-awesome-icon icon="plus" class="mr-1.5 h-4 w-4" /> 
        <span class="hidden xs:inline">Add Job</span>
        <span class="xs:hidden">Job</span>
      </button>
    </div>
  </header>
</template>
