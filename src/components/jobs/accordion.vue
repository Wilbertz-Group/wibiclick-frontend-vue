<script setup>
	...
	const props = defineProps(['jobs', 'status', 'user'])
...
<template>
	<div v-for="(job) in jobs" :key="job.uid" :id="dateTimestamp(job.createdAt)" class="shadow rounded-lg sm:overflow-hidden bg-white mb-4">
		<h2 :id="'accordion-job-heading-'+job.uid">
			<button type="button" class="flex items-center justify-between w-full px-3 py-1 border-white " data-accordion-target="#accordion-arrow-icon-body-2" aria-expanded="false" aria-controls="accordion-arrow-icon-body-2">
				<span class="flex items-center">
					<div class="flex items-center -space-x-4 hover:space-x-1" data-v-2fc82866-s="" data-v-2fb1486c-s="">
						<button type="button" class="">
							<svg id="tooltip-job-button" data-tooltip-target="tooltip-job" class="w-8 h-8 cursor-pointer rounded-full shadow-lg bg-slate-900 hover:bg-slate-700 p-2 text-white" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
				...
				<div class="flex items-center">
					<span class="ml-5 text-xs italic">{{ dateFormatter(job.createdAt) }}</span>
		...
		<div :id="'accordion-job-body-'+job.uid" class="hidden" aria-labelledby="accordion-arrow-icon-heading-2">
			<div class="px-3 py-1 rounded-b-md font-light border border-b-0 border-gray-200 dark:border-gray-700">
				<p class="mb-2 text-black dark:text-gray-400">{{ status }} by <b>{{ user }}</b> at {{ dateFormatter(job.createdAt) }}</p>
			</div>
		</div>
	</div> 
</template>