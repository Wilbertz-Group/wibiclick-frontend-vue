...
const showAdvancedMetrics = ref(true)

// Fetch monthly trends data for context (last 6 months)
const fetchMonthlyTrends = async () => {
  if (!props.websiteId || props.websiteId === 'default') return
  
  loadingTrends.value = true
  try {
    // In a real implementation, fetch trends from your API
    // For demo purposes, create mock data based on current overview
    
    // Get base values from props
    const baseEarnings = props.financialOverview?.totalEarnings?.current || 10000
    const baseExpenses = props.financialOverview?.totalExpenses?.current || 5000
    const baseProfit = props.financialOverview?.netProfit?.current || 5000
    
    const trends = { earnings: [], expenses: [], profit: [] }
    
    // Generate 6 months of data (including current month)
    for (let i = 5; i >= 0; i--) {
      // Calculate month index (current month - i)
      const date = new Date(props.year, props.month - 1 - i)
      const monthName = date.toLocaleString('default', { month: 'short' })
      
      // Generate realistic variations (more stable for expenses, more variable for earnings)
      const variationEarnings = 0.9 + (Math.random() * 0.2) // 0.9-1.1 variation
      const variationExpenses = 0.95 + (Math.random() * 0.1) // 0.95-1.05 variation
      
      // Add slight upward trend
      const trendFactor = 1 + (i * 0.01)
      
      // Calculate values with variations
      const monthEarnings = Math.round(baseEarnings * variationEarnings / trendFactor)
      const monthExpenses = Math.round(baseExpenses * variationExpenses / trendFactor)
      const monthProfit = monthEarnings - monthExpenses
      
      // Add to trends arrays
      trends.earnings.push({ x: monthName, y: monthEarnings })
      trends.expenses.push({ x: monthName, y: monthExpenses })
      trends.profit.push({ x: monthName, y: monthProfit })
    }
    
    // Set current month values to match exactly
    if (trends.earnings.length > 0) {
      trends.earnings[trends.earnings.length - 1].y = baseEarnings
      trends.expenses[trends.expenses.length - 1].y = baseExpenses
      trends.profit[trends.profit.length - 1].y = baseProfit
    }
...
// Earnings distribution radial chart options
const earningsRadialOptions = computed(() => ({
  chart: {
    type: 'radialBar',
  ...
  plotOptions: {
    radialBar: {
      hollow: {
        size: '65%',
        background: props.isDarkMode ? '#1f2937' : '#ffffff',
        image: undefined,
        margin: 10
      },
      track: {
  ...
  tooltip: {
    ...
    y: {
      formatter: (val) => {
        // Calculate the actual value from percentage
        const total = (props.financialOverview?.companyEarnings?.current || 0) + 
                    (props.financialOverview?.technicianEarnings?.current || 0);
        return formatCurrency((val * total) / 100);
      },
      title: {
        formatter: (seriesName) => seriesName + ' Earnings'
      }
    }
  }
}))

// Earnings radial chart series (percentages)
const earningsRadialSeries = computed(() => {
  if (!props.financialOverview?.companyEarnings?.current && !props.financialOverview?.technicianEarnings?.current) {
    return [0, 0];
  }
  
  const companyEarnings = props.financialOverview?.companyEarnings?.current || 0;
  const technicianEarnings = props.financialOverview?.technicianEarnings?.current || 0;
  const total = companyEarnings + technicianEarnings;
  
  if (total === 0) return [0, 0];
  
  // Calculate percentages for radial bars
  const companyPercentage = Math.round((companyEarnings / total) * 100);
  const technicianPercentage = Math.round((technicianEarnings / total) * 100);
  
  return [companyPercentage, technicianPercentage];
})

// Financial trends chart options
const trendsChartOptions = computed(() => ({
  ...
  colors: ['#4f46e5', '#ef4444', '#10b981'], // Indigo (revenue), Red (expenses), Green (profit)
  ...
  grid: {
    ...
    row: { colors: [props.isDarkMode ? 'transparent' : 'transparent'], opacity: 0.5 }
  },
  markers: {
    size: 4,
  ...
  xaxis: {
    categories: monthlyTrends.value.earnings.map(item => item.x),
    labels: {
...
const trendsChartSeries = computed(() => {
  return [
    {
      name: 'Earnings',
      data: monthlyTrends.value.earnings.map(item => item.y)
    },
    {
      name: 'Expenses',
      data: monthlyTrends.value.expenses.map(item => item.y)
    },
    {
      name: 'Profit',
      data: monthlyTrends.value.profit.map(item => item.y)
    }
  ]
})
...
watch(() => props.financialOverview, fetchMonthlyTrends)
watch(() => props.websiteId, fetchMonthlyTrends)
