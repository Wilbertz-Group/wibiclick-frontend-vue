<!-- wibiclick-frontend-vue/src/components/invoices/accordion.vue -->
<script setup>
...
<template>
	<div v-for="(invoice) in invoices" :key="invoice.uid" :id="dateTimestamp(invoice.createdAt)">	
		<span class="absolute flex items-center justify-center w-6 h-6 bg-blue-200 rounded-full left-0 ring-white dark:ring-gray-900 dark:bg-blue-900">
			<svg id="tooltip-invoice-button" class="cursor-pointer rounded-full shadow tag-content-invoice hover:bg-slate-700 p-1 text-white" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
		...
		<div class="shadow relative rounded-lg sm:overflow-hidden bg-white mb-4">
			...
			<span class="tag-content-invoice tag-content-style tag-hover-effect"></span>
		</div> 
	</div>
</template>