...
<script setup>
	...
	const props = defineProps(['estimates', 'status', 'user', 'created'])
...
<template>
	<div v-for="(estimate) in estimates" :key="estimate.uid" :id="dateTimestamp(created)">	
		<span class="absolute flex items-center justify-center w-6 h-6 bg-blue-200 rounded-full left-0 ring-white dark:ring-gray-900 dark:bg-blue-900">
			<svg id="tooltip-estimate-button" class="cursor-pointer rounded-full shadow tag-content-estimate hover:bg-slate-700 p-1 text-white" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
		...
		<div class="shadow relative rounded-lg sm:overflow-hidden bg-white mb-4">
			...
			<div :id="'accordion-estimate-body-'+estimate.uid" class="hidden" aria-labelledby="accordion-arrow-icon-heading-2">
				<div class="px-3 py-1 rounded-b-md font-light border border-b-0 border-gray-200 dark:border-gray-700">
					<p class="p-2 my-2 text-xs italic font-normal text-black border border-gray-200 rounded-lg dark:border-gray-500 dark:text-gray-300">#{{ estimate.number }}: {{ status }} by <b>{{ user }}</b> on {{ dateFormatter(created) }}</p>
				</div>
			</div>
			<span class="tag-content-estimate tag-content-style tag-hover-effect"></span>
		</div> 
	</div>
</template>