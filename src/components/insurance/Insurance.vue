<!-- wibiclick-frontend-vue/src/components/insurance/Insurance.vue -->
<script setup>
  ...
  ListboxOption,
	...
	const currentStatus = statuses.filter(a => a.value == props.insurance.status)?.length && props.insurance?.status ? statuses.filter(a => a.value == props.insurance.status) : ''
	...
	watch(selectedStatus, async (n, o) => {
		toast.success('updating the status of the insurance report')
		body.value = `Insurance report status changed from <b>${o.value}</b> to <b>${n.value}</b>`
		heading.value = "Insurance report status"
		
		const data = { 
			id: props.insurance.id, 
			status: n.value, 
			customerId: props.insurance.customerId, 
			employeeId: props.insurance.employeeId
		}
	...
	<div class="shadow rounded-xl px-3 pt-3 pb-1 mt-3 cursor-move w-full dark:bg-[#ffffff]">
		<div class="mb-2 p-0">
			...
			<div class="flex justify-between">
				<p class="text-sm font-bold text-black" >Report number</p>
				<p class="text-sm text-black" >#{{ insurance.number }}</p>
			</div>
			<div class="flex justify-between" >
				<p class="text-sm font-bold text-black" >Date</p>
				<p class="text-sm text-black" >{{ universalDateFormatter(insurance.createdAt).includes(':') ? 'Today at '+ universalDateFormatter(insurance.createdAt) : universalDateFormatter(insurance.createdAt) }}</p>
			</div>
		</div>
		<div class="flex mt-4 mb-1 justify-between items-center" >
			<router-link :to="{ name: 'edit-insurance-report', query:{ insurance_id: insurance.id } }" class="text-white cursor-pointer inline-block bg-slate-900 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-gray-300 dark:focus:ring-gray-800 shadow-lg shadow-gray-500/50 dark:shadow-lg dark:shadow-gray-800/80 font-medium rounded-lg text-sm px-3 py-1 text-center"> Edit </router-link>
			<router-link :to="{ name: 'view-insurance-report', query:{ insurance_id: insurance.id } }" class="text-white cursor-pointer inline-block bg-slate-900 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-gray-300 dark:focus:ring-gray-800 shadow-lg shadow-gray-500/50 dark:shadow-lg dark:shadow-gray-800/80 font-medium rounded-lg text-sm px-3 py-1 text-center"> View </router-link>
		</div>
	</div>
</template>