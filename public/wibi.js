function createWidget(n) {
  ...
  !(async function () {
    ...
    var contact_utk =
      ...
      utk = contact_utk || "",
      P = await fetch(
        "https://wibi.wilbertzgroup.com/wibi-options?id=${n}&c=0&pg=${pg}&utk=${utk}"
      ),
      B = await P.json();
    (m = B.name),
      (f = B.designation),
    ...
    try {
      v.map((n, e) => {
        ...
        var o = "";
        n.logo_code &&
          (o = '<i class="fa customButtonIcon">&#x${n.logo_code};</i>'),
          (t.id = "customButton${e}"),
          (t.innerHTML =
            '              <div class="chatPopup__phoneText">${o}${n.label}</div>');
        var a = document.createElement("style");
        (a.innerHTML =
          '.customButtonIcon{            margin-right: 6px;          }           #customButton${e} {              background-color: #${n.colorCode.replace("#","")}0D;              color: #${n.colorCode};              display: flex;              border-radius: 24px;              padding: 8px 16px;              fill: #${n.colorCode.replace("#","")};              text-decoration: none;              justify-content: center;              margin-top: 16px;              width: 100%;            }                        #customButton${e}:hover {              background-color: #${n.colorCode.replace("#","")};              color: white;              display: flex;              border-radius: 24px;              padding: 8px 16px;              fill: white;              text-decoration: none;              justify-content: center;              margin-top: 16px;            }'),
          k.appendChild(a),
          document.getElementById("divContainer").appendChild(t);
      });
    } catch (n) {}

    async function C() {
      "none" == document.getElementById("myForm").style.display
        ? ((document.getElementById("myForm").style.display = "block"),
          (document.getElementById("openButton__closeIcon").style.display =
            "block"),
          (document.getElementById("openButton__phoneIcon").style.display =
            "none"),
    ...
    async function F(e) {
      ...
      var Q = await fetch(
          "https://wibi.wilbertzgroup.com/wibi-action?id=${n}&c=3&ic=${text}&pg=${pg}&utk=${utk}"
        ),
        Y = await Q.json();
    ...
    v && v.length,
      window.location.href.search("#OpenClick2Contact") >= 0 && C(),
      (document.getElementById("openButton").onclick = function () {
        C();
      }),
      document.querySelectorAll(".wibiclickcss div#divContainer a").forEach(
        (button) =>
